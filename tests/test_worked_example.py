...
def test_worked_example_page_numbers():
    ...
    assert crm.mean() == pytest.approx(2_475_636, rel=1e-3)
    ...
    assert rs.metrics.tvar(res.retained_losses, 0.99) == pytest.approx(3_200_000.0, rel=1e-3)
    ...
    assert pe.at(0.0).margin_rate == pytest.approx(2.0966, rel=2e-2)  # dollars per unit
